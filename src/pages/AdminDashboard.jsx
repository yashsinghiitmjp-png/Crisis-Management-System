...
const AdminDashboard = () => {
  const { incidents, loading, escalateIncident, getAIRecommendations } = useRealtimeIncidents();
  ...
  const handleResolve = async (incidentId) => {
    ...
    try {
      ...
      if (incident && incident.tasks) {
        for (const task of incident.tasks) {
          await update(ref(db, `staff/${task.staff_id}`), { isAvailable: true });
        }
      }
    } catch (e) {
              ...
              activeIncidents.map(incident => (
                <div 
                  ...
                  style={{
                    ...
                    border: '1px solid',
                    borderColor: selectedId === incident.id ? 'rgba(59, 130, 246, 0.2)' : 'transparent',
                    transition: 'all 0.2s',
                    position: 'relative',
                    overflow: 'hidden'
                  }}
              ...
              <div className="card" style={{ flexShrink: 0 }}>
                <div className="flex justify-between items-start" style={{ marginBottom: '1.5rem' }}>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button className="btn" style={{ background: 'var(--bg-main)' }}>
                ...
                <div className="grid gap-3">
                  {selectedIncident.tasks && selectedIncident.tasks.length > 0 ? (
                    selectedIncident.tasks.map(task => {
                      ...
                      return (
                        <div key={task.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem', background: 'rgba(255,255,255,0.03)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-light)' }}>
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <div style={{ fontWeight: 700 }}>{assignedUser?.name || 'Unit ' + task.staff_id}</div>
                              <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>Status: {task.status.toUpperCase()}</div>
                            </div>
                          </div>
                          <div className="flex items-center gap-3">
                             <span style={{ fontSize: '0.75rem', color: task.acknowledged ? 'var(--color-success)' : 'orange' }}>
                               {task.acknowledged ? '✓ ACKNOWLEDGED' : '○ PENDING ACK'}
                             </span>
                    ...
                    <div style={{ textAlign: 'center', padding: '1.5rem', color: 'var(--color-critical)', border: '1px dashed rgba(239, 68, 68, 0.3)', borderRadius: 'var(--radius-md)' }}>
                      CRITICAL: No units assigned to this incident!
                    </div>
                  )}
                </div>
              </div>
